...
class ThingsBoardSized
{
  ...
  public:
    ...
    bool connect(const char *host, const char *access_token = "provision", int port = 1883, const char *client_id = "TbDev", const char *password = NULL, int updates_port = 8080) {
      if (!host) {
        return false;
      }
      ...
      thingsboardHost = String(host);
      thingsboardPort  = port;
      updatesPort = updates_port;
    ...
    JsonObject Request_Client_Attributes(char *attributes_list[], int attributes_list_size) {
      String request_link;
      request_link = "http://" + thingsboardHost + ":" + updatesPort + "/api/v1/" + accessToken + "/attributes?clientKeys=";
      for (int i = 0; i < attributes_list_size; i++) {
        request_link += String(attributes_list[i]);
        if (i < attributes_list_size - 1) {
          request_link += ",";
        }
      }
      return send_get_request(request_link.c_str());
    }

    JsonObject Request_Shared_Attributes(char *attributes_list[], int attributes_list_size) {
      String request_link;
      request_link = "/api/v1/" + accessToken + "/attributes?sharedKeys=";
      for (int i = 0; i < attributes_list_size; i++) {
        request_link += String(attributes_list[i]);
        if (i < attributes_list_size - 1) {
          request_link += ",";
    ...
    static void update_started() {
      Logger::log("HTTP update process started");
    }

    static void update_finished() {
      Logger::log("HTTP update process finished");
    }

    static void update_progress(int cur, int total) {
      Logger::log("HTTP update process at " + String(cur) + " of " + String(total) + " bytes...\n");
    }

    static void update_error(int err) {
      Logger::log("HTTP update fatal error code " + String(err) + "\n");
    }

    JsonObject Request_Firmware_Info() {
      char* target_attributes[] = {"fw_checksum", "fw_checksum_algorithm", "fw_size", "fw_tag", "fw_title", "fw_version"};
      int target_attributes_count = 6;

      return Request_Shared_Attributes(target_attributes, target_attributes_count);
    }

    void update(const char *host, int port, const char *fw_title, const char *fw_version) {
      String firmware_link = "http://" + String(host) + ":" + String(port) + "/api/v1/" + String(accessToken) + "/firmware?title=" + String(fw_title) + "&version=" + String(fw_version);
      #ifdef ESP8266

      ESPhttpUpdate.setLedPin(LED_BUILTIN, LOW);

      ESPhttpUpdate.onStart(ThingsBoardSized::update_started);
      ESPhttpUpdate.onEnd(ThingsBoardSized::update_finished);
      ESPhttpUpdate.onProgress(ThingsBoardSized::update_progress);
      ESPhttpUpdate.onError(ThingsBoardSized::update_error);

      ESPhttpUpdate.update(firmware_link.c_str());
      #else
  ...
  private:
    ...
    JsonObject send_get_request(const char* request_link) {
      ...
      // #endif
      if (!http.connected()) {
        if (http.connect(thingsboardHost.c_str(), updatesPort)) {
          int httpCode = http.get(request_link);
          Logger::log(String("Sending request to " + String(request_link) + ", result: " + String(httpCode)));
          if (httpCode == 0) {
              DeserializationError error = deserializeJson(jsonBuffer, http.responseBody());
              if (error) {
                Logger::log("Unable to de-serialize response");
                Logger::log(error.c_str());
              }
          } else {
            Logger::log(String("GET... failed, error:" + String(http.responseStatusCode())));
          }
          http.stop();
        }
      }
      JsonObject resp_obj = jsonBuffer.template as<JsonObject>();
      return resp_obj["shared"];
    }
