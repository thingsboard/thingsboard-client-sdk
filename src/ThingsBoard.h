...
public:
  ...
  inline ThingsBoardSized(const bool &enableQoS = false)
      : m_client(), m_maxStack(Default_Max_Stack_Size), m_rpcCallbacks(), m_rpcRequestCallbacks(), m_sharedAttributeUpdateCallbacks(), m_attributeRequestCallbacks(), m_provisionCallback(), m_requestId(0U), m_qos(enableQoS)
#if defined(ESP8266) || defined(ESP32)
        ,
        m_fwState(false), m_fwCallback(nullptr), m_fwSize(0U), m_fwChecksumAlgorithm(), m_fwAlgorithm(), m_fwChecksum(), m_fwSharedKeys{FW_CHKS_KEY, FW_CHKS_ALGO_KEY, FW_SIZE_KEY, FW_TITLE_KEY, FW_VER_KEY}, m_fwRequestCallback(m_fwSharedKeys.cbegin(), m_fwSharedKeys.cend(), std::bind(&ThingsBoardSized::Firmware_Shared_Attribute_Received, this, std::placeholders::_1)), m_fwUpdateCallback(m_fwSharedKeys.cbegin(), m_fwSharedKeys.cend(), std::bind(&ThingsBoardSized::Firmware_Shared_Attribute_Received, this, std::placeholders::_1)), m_fwChunkReceive(0U)
#endif
  {
#if !THINGSBOARD_ENABLE_STL
