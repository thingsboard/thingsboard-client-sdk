...
/// @tparam MaxRPC Maximum amount of key-value pairs that will ever be sent with the RPC_Callback, allows to use a StaticJsonDocument on the stack in the background.
  ...
  private:
...
#else
    Array<RPC_Callback<MaxRPC>, MaxSubscribtions>                      m_rpc_callbacks;                     // Server side RPC callbacks array
    ...
    inline void process_rpc_message(char * const topic, JsonObjectConst const & data) {
        ...
        for (RPC_Callback & rpc : m_rpc_callbacks) {
            ...
            JsonDocument & document = rpc.Get_Json_Document();
            rpc.template Call_Callback<Logger>(param, document);
            ...
            size_t const jsonSize = Helper::Measure_Json(document);
            Send_Json(responseTopic, document, jsonSize);
            break;
        }
    }
