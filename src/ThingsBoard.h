...
/// @tparam MaxEndpointsAmount Maximum amount of subscribed API endpoints, Default_Endpoints_Amount is used as the default because the OTA Firmware API, default = Default_Endpoints_Amount (7)
/// @tparam Logger Implementation that should be used to print error messages generated by internal processes and additional debugging messages if THINGSBOARD_ENABLE_DEBUG is set, default = DefaultLogger
  ...
  private:
    ...
    void onMQTTMessage(char * const topic, uint8_t * const payload, unsigned int length) {
        ...
        for (const auto & api : m_api_implementations) {
            // Process all possible api responses that require the data unserialized.
            // If it was processed unserialized, we will not process it serialized
