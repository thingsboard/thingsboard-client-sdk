...
template<size_t MaxFieldsAmount = Default_Fields_Amount,
         size_t MaxSubscribtions = Default_Subscriptions_Amount,
  ...
  public:
    ...
    inline bool Send_Json(const char* topic, const TSource& source, const size_t& jsonSize) {
      ...
      if (MaxFieldsAmount < amount) {
        char message[Helper::detectSize(TOO_MANY_JSON_FIELDS, amount, MaxFieldsAmount)];
        snprintf_P(message, sizeof(message), TOO_MANY_JSON_FIELDS, amount, MaxFieldsAmount);
        Logger::log(message);
        return false;
      }
#endif // !THINGSBOARD_ENABLE_DYNAMIC
