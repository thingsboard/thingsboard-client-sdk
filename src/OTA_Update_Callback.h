...
class OTA_Update_Callback : public Callback<void, bool const &> {
  public:
    ...
    using progress_function = std::function<void(size_t const & current, size_t const & total)>;
#else
    using progress_function = void (*)(size_t const & current, size_t const & total);
    ...
    OTA_Update_Callback(function endCb, char const * const currFwTitle, char const * const currFwVersion, IUpdater * const updater, uint8_t const & chunkRetries = CHUNK_RETRIES, uint16_t const & chunkSize = CHUNK_SIZE, uint64_t const & timeout = REQUEST_TIMEOUT);
    ...
    /// @param timeout Maximum amount of time in microseconds for the OTA firmware update for each seperate chunk,
    /// until that chunk counts as a timeout, retries is then subtraced by one and the download is retried
    OTA_Update_Callback(progress_function progressCb, function endCb, char const * const currFwTitle, char const * const currFwVersion, IUpdater * const updater, uint8_t const & chunkRetries = CHUNK_RETRIES, uint16_t const & chunkSize = CHUNK_SIZE, uint64_t const & timeout = REQUEST_TIMEOUT);
    ...
    void Set_Progress_Callback(progress_function progressCb);
    ...
    uint8_t const & Get_Chunk_Retries() const;
    ...
    void Set_Chunk_Retries(uint8_t const & chunkRetries);
    ...
    uint16_t const & Get_Chunk_Size() const;
    ...
    void Set_Chunk_Size(uint16_t const & chunkSize);
  ...
  private:
    progress_function m_progressCb; // Progress callback to call
    char const        *m_fwTitel;   // Current firmware title of device
    char const        *m_fwVersion; // Current firmware version of device
