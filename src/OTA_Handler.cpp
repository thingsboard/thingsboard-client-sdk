...
void OTA_Handler::Stop_Firmware_Update() {
    m_watchdog.detach();
    m_fw_updater->reset();
    m_logger.logf(FW_UPDATE_ABORTED);
...
void OTA_Handler::Process_Firmware_Packet(const size_t& current_chunk, uint8_t *payload, const size_t& total_bytes) {
    ...
    if (current_chunk != m_requested_chunks) {
      m_logger.logf(RECEIVED_UNEXPECTED_CHUNK, current_chunk, m_requested_chunks);
      return;
    }

    m_watchdog.detach();
#if THINGSBOARD_ENABLE_DEBUG
    m_logger.logf(FW_CHUNK, current_chunk, total_bytes);
#endif // THINGSBOARD_ENABLE_DEBUG

    if (current_chunk == 0U) {
        // Initialize Flash
        if (!m_fw_updater->begin(m_fw_size)) {
          m_logger.logf(ERROR_UPDATE_BEGIN);
...
void OTA_Handler::Request_Next_Firmware_Packet() {
    ...
    if (!m_publish_callback(m_requested_chunks)) {
      m_logger.logf(UNABLE_TO_REQUEST_CHUNCKS);
...
#if THINGSBOARD_ENABLE_DEBUG
    m_logger.logf(HASH_ACTUAL, calculated_hash);
    m_logger.logf(HASH_EXPECTED, m_fw_checksum);
#endif // THINGSBOARD_ENABLE_DEBUG
