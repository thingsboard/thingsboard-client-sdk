...
// Wheter the given script generates the deviceName from the included mac address on the WiFi chip,
...
void loop() {
  ...
  if (!provisionRequestSent) {
    if (!tb.connected()) {
      ...
      (DEVICE_NAME != NULL) && (DEVICE_NAME[0] == '\0') ? WiFi.macAddress().c_str() : DEVICE_NAME;
#else
