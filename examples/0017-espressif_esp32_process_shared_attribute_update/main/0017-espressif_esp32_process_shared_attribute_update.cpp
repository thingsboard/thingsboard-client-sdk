...
static void on_got_ip(void* arg, esp_event_base_t event_base,
    ...
    ESP_LOGI("Attribute Update", "Got IP address");
}
    ...
    while (true) {
        if (!tb.connected()) {
            ...
            if (tb.connect(THINGSBOARD_SERVER, TOKEN, THINGSBOARD_PORT)) {
                ESP_LOGI("Attribute Update", "Successfully connected");
                ...
                if (!subscribed) {
                    ...
                    if (tb.Shared_Attributes_Subscribe(processSharedAttributeUpdate, subscribed_attributes, 2)) {
                        ...
                        ESP_LOGE("Attribute Update", "Failed to subscribe for shared attribute updates");
                    }
                }
            } else {
                ESP_LOGE("Attribute Update", "Failed to connect");
            }
        }
